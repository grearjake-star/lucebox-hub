...
int main(int argc, char **argv) {
    if (argc < 3 || argc > 4) {
        std::cerr << "usage: " << argv[0]
                  << " <qwen3.5-0.8b-safetensors-or-dir> <seq_len<=2048> [max_seq_len]\n";
        return 2;
    }
    ...
    if (seq_len > 2048 || max_seq_len > 2048 || (seq_len % 32) != 0) {
        std::cerr << "smoke_prefill_megakernel requires seq_len/max_seq_len <= 2048 and seq_len multiple of 32\n";
        return 2;
    }
    ...
    launch_prefill_bf16_mega(
        ids_dev, seq_len, static_cast<int *>(ctx.output_token),
